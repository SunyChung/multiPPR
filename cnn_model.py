...
class FeatureEmb(nn.Module):
    ...
    def forward(self, user_idxs, item_idxs):
        # print('user_idxs shape : ', user_idxs.shape)  # (3516,) -> (3515,) -_;;
        user_neighs = self.user_idx_tensor[user_idxs]
        # print('\nuser_neighs shape : ', user_neighs.shape)
        # torch.Size([3515 (=batch_size=# of items), multi_factor x top_k])
        neigh_emb = self.user_emb(user_neighs)
        # print('neigh_emb shape : ', neigh_emb.shape)
        # torch.Size([3515 (=batch_size), multi_factor x top_k, embedding_dim])
        neigh_score = self.user_scr_tensor[user_neighs]
        # print('neigh_score shape : ', neigh_score.shape)
        # torch.Size([3515 (=batch_size), multi_factor x top_k, multi_factor x top_k])
        ...
        for i in range(self.multi_factor):
            ...
            # torch.Size([3515, 20, 64])
            summed_user[i] = torch.sum(scored_user_emb[:, self.top_k * i: self.top_k * (i+1), :], dim=1)
            # print('summed_user shape: ', summed_user[i].shape)  # torch.Size([3515, 64])
        ...
        # print('\ncon_user_emb shape : ', concat_user_emb.shape)  # torch.Size([3515, 320])
        # stack makes a new dimension
        # print('stacked_user_emb shape : ', stacked_user_emb.shape)  # torch.Size([3515, 5, 64])
        ...
        concat_item_emb = torch.cat(list(summed_item.values()), dim=1)
        stacked_item_emb = torch.stack(list(summed_item.values()), dim=1)  # torch.Size([3515, 5, 64])

        concat_user_item = torch.mm(concat_user_emb, concat_item_emb.T)
        # print('concat_user_item shape : ', concat_user_item.shape)
        stacked_user_item = torch.bmm(stacked_user_emb, stacked_item_emb.permute(0, 2, 1))
        # print('stacked_user_item shape : ', stacked_user_item.shape)  # torch.Size([3515, 5, 5])
        # print('len(stacked_user_item) : ', len(stacked_user_item))

        results = []
        for i in range(len(stacked_user_item)):
            print('stacked_user_item : ', stacked_user_item[i].shape)  # [5, 5]
            print('input shape : ', (stacked_user_item[i].reshape(1, 1, -1, -1)).shape)
            #  torch.Size([1, 1, 5, 5])
            result = torch.sigmoid(
                self.c_net(stacked_user_item[i].view(1, 1, -1, -1)))
                # self.c_net(stacked_user_item[i, :, :].unsqueeze_(0).unsqueeze_(0)))
            # print('result shape : ', result.shape)
            # torch.Size([1])
            # print('result : ', result)
            # tensor([0.8552], device='cuda:0', grad_fn=<SigmoidBackward>)
            results.append(result)
        # print('results : ', results)
        # ... , grad_fn=<SigmoidBackward>), tensor([0.9992], grad_fn=<SigmoidBackward>)]
        # print('results shape : ', len(results))  # 3515
        # return torch.Tensor(results)
        # return results  # 리스트로 반환하면, gradient tag 가 사라져서 loss 연산 불가 !! 주의 !!
...
if __name__ == '__main__':
    ...
    femb = FeatureEmb(item_idx_tensor, item_scr_tensor,
                      ...
                      n_items, top_k, multi_factor, emb_dim)
    results \
        = femb(np.random.randint(0, 6000, 3515), np.random.randint(0, 3000, 3515))
